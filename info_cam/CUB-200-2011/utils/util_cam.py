...
def get_cam(model, target=None, input=None, args=None):
    ...
    with torch.no_grad():
        ...
        basic_cam_weight = cam_weight

        # print('args sub oth cams: ', args.sub_oth_cams)
        if args.max_cam:
        ...
        # cam_weight = basic_cam_weight - cam_weight
        ...
        if args.conv_cams:
            cam_filter = torch.ones(1, channel, 4, 4).to(target.device)
            cam = F.conv2d(cam, cam_filter, padding=3)
        else:
