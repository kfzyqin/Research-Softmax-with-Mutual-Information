...
from data.cub_200.cub_data_tool import normed_class_prob
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        def clone_fn(batch_queue):
            ...
            aux_ends = tf.exp(end_points['AuxLogits'])
            sum_exp_aux = tf.reduce_sum(aux_ends, axis=-1, keep_dims=True)
            tmp_loss = tf.reduce_sum(sum_exp_aux, axis=-1)
            soft_max_aux = tf.reduce_sum(-tf.log((aux_ends / sum_exp_aux) * labels), axis=-1)

            logit_end = tf.exp(logits)

            # Switch these two
            logit_end_denom = tf.exp(logits) * normed_class_prob  # PC-softmax
            # logit_end_denom = tf.exp(logits)  # Traditional-softmax

            sum_exp = tf.reduce_sum(logit_end_denom, axis=-1, keep_dims=True)
            soft_max = -tf.log(tf.reduce_sum((logit_end / sum_exp) * labels, axis=-1))
            ...
            # if 'AuxLogits' in end_points:
            #     tf.losses.compute_weighted_loss(tmp_loss, weights=0.4, scope='aux_loss',
            ...
            # tf.losses.softmax_cross_entropy(
            #     labels, logits, label_smoothing=FLAGS.label_smoothing, weights=1.0)
            tf.losses.compute_weighted_loss(soft_max, weights=1.0, scope='aux_loss',
                                            loss_collection=ops.GraphKeys.LOSSES,
                                            reduction=Reduction.SUM_BY_NONZERO_WEIGHTS)
            return end_points
